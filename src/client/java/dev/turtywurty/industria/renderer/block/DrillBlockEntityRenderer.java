...
public class DrillBlockEntityRenderer extends IndustriaBlockEntityRenderer<DrillBlockEntity> {
    ...
    @Override
    protected void onRender(DrillBlockEntity entity, float tickDelta, MatrixStack matrices, OrderedRenderCommandQueue queue, int light, int overlay) {
        ...
        { // Render motor
            if (!entity.getMotorInventory().isEmpty()) {
                ...
                queue.submitModel(this.motorModel, null, matrices, this.motorModel.getLayer(DrillMotorModel.TEXTURE_LOCATION), light, overlay, -1, null);
        ...
        { // Render frame
            ...
            queue.submitModel(model, null, matrices, this.model.getLayer(DrillFrameModel.TEXTURE_LOCATION), light, overlay, -1, null);
        ...
        { // Render cable wheel
            ...
            queue.submitModel(cableModel, null, matrices, this.cableModel.getLayer(DrillCableModel.TEXTURE_LOCATION), light, overlay, -1, null);
            cableMain.xScale += cableScaleFactor;
            cableMain.yScale += cableScaleFactor;
            cableMain.zScale += cableScaleFactor;
        ...
        { // Render drill cable
            ...
            queue.submitCustom(matrices, RenderLayer.getLines(), (entry, linesVertexConsumer) -> {
        ...
        { // Render drill head
            ...
            drillHeadData.onRender().render(entity, drillHeadStack, tickDelta, matrices, queue, drillHeadModel, drillHeadModel.getLayer(drillHeadTexture), light, overlay);
            matrices.pop();
        }
    }

    @Override
    protected void postRender(DrillBlockEntity entity, float tickDelta, MatrixStack matrices, OrderedRenderCommandQueue queue, int light, int overlay) {
        Box aabb = entity.getDrillHeadAABB();
        if (MinecraftClient.getInstance().debugHudEntryList.isEntryVisible(DebugHudEntries.ENTITY_HITBOXES) && aabb != null) {
            ...
            queue.submitCustom(matrices, RenderLayer.getLines(), (matricesEntry, vertexConsumer) -> {
                VertexRendering.drawBox(matricesEntry, vertexConsumer, minX, minY, minZ, maxX, maxY, maxZ, 1, 0, 0, 1);
            });
        }
    }
